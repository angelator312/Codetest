...
export async function SubmitCode(filePath, opions = {}) {
  ...
  try {
    ...
    if (
      ...
      typeof judge.reloadToken === "function"
    ) {
      const cred = judge.reloadAuth(config.getJudgeCredentials(judge.name));
      config.setJudgeCredentials(judge.name, cred);
      response = await judge.submit(code, problemId, credentials);
    }
